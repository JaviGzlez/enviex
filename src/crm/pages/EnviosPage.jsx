...
export default function EnviosPage() {
  ...
  const loadShipments = async (forDate) => {
    setLoading(true);
    const { data, error } = await supabase
      .from("shipments")
      .select("id, shipment_date, service_type, price, notes, companies(name), profiles(full_name)")
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-slate-500">
            {loading ? "Cargando..." : `${shipments.length} envío(s) este día`}
          </span>
        </div>
        <button
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="grid grid-cols-[1.6fr_1fr_0.8fr_0.8fr] bg-slate-50 px-4 py-2 text-xs font-bold text-slate-400">
          <div>Empresa</div>
        ...
        {shipments.map((s) => (
          <div key={s.id} className="grid grid-cols-[1.6fr_1fr_0.8fr_0.8fr] border-t border-slate-100 px-4 py-3 text-sm">
...
function NewShipmentModal({ date, companies, profileId, onClose, onCreated }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-3xl bg-white p-6" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-black text-[#092640]">Nuevo envío · {date}</h2>
